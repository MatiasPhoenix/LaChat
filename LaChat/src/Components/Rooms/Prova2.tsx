...
function Prova2() {
  ...
  // Gestione degli elementi salvati nel localStorage
  ...
  // Salva nel database i messaggi scambiati tra utenti
  ...
  // Imposta immagine avatar confrontando database e immagini salvate
  ...
  return (
    <div className="container homepage bg-slate-950">
      <div className="flex items-center py-6 bg-slate-800 h-10">
        ...
        <img className=" me-3 h-8" src={chooseAvatar(imgUser!)} alt="" />
      ...
      <div className="mt-5 areaChat mx-5">
        {messages.map((msg, index) => (
          <div key={index} className="mb-2 grid justify-items-stretch">
            ...
            ${
              msg.sender === userId
                ? "rounded-tr-none rounded-tl-lg bg-green-800 justify-self-end text-left pl-6"
                : "rounded-tl-none bg-sky-800 justify-self-start text-right pr-6"
            }`}
            >
              <p
                ref={messagesEndRef}
